...
網友回報 32K 的 decode 中位數是 113.1 tps；我用預設 `cooldown=2` 量到 89.7。
那個 cooldown 下的逐次數字是 98.8 → 94.9 → 89.7 → 86.1 → 82.7——5 次跑連續
單調下滑，看起來像是熱降頻或多次跑同樣工作負載累積的 cache 壓力。
所以我又用 `cooldown=60`（網友的方法論）重跑一次，得到 121.2 tps、std=0.2——
基本上對上他們的 123，證實了原本的落差是短 cooldown 造成的熱降頻 artifact。
[cooldown=2 vs cooldown=60 比較](#cooldown-比較2-秒-vs-60-秒) 在下方獨立一段。

### Cooldown 比較（2 秒 vs 60 秒）

同樣的 dflash-mlx v0.1.7 server、同樣的 prompt、同樣每格 5 次——只有 run
之間的 cooldown 改了。原始資料：[`data/dflash_v6.jsonl`](../data/dflash_v6.jsonl)
（2 秒）跟 [`data/dflash_v6_c60.jsonl`](../data/dflash_v6_c60.jsonl)（60 秒）。
...
短脈絡那幾格沒變——這些尺寸每次跑都在 3 秒內結束，M5 Max 就算 cooldown 只有
2 秒也有足夠的熱裕度。16K 跟 32K 那幾格——每次跑超過 10 秒的持續 GPU 工作——
就出現明顯分裂：cooldown 只有 2 秒時晶片會逐漸降頻（std=6.5，來自單調下滑），
但給 60 秒恢復時間後每次跑幾乎一模一樣（std=0.2）。

benchmarking 的教訓：在 Apple Silicon 上跑長脈絡時，cooldown 是一個實質的
方法論變數。32K cooldown=2 中位數的 89.7 代表「熱壓力下的穩態」，
而 cooldown=60 的 121.2 則代表「單次請求的峰值表現」。兩個都是有效的測量，
只是測的是不同的東西。網友的設定剛好用 cooldown=60（所以他們的 123 tps
比較接近我們的 121.2 而不是 89.7），對「這個框架到底有多快？」這個問題來說
這是比較好的回報數字；而 cooldown=2 比較反映「在背靠背負載下它能交付什麼？」。
這個 lab 其他框架的測試全都是 cooldown=2，所以跨框架比較時 v6 的數字
才是 apples-to-apples 的；v6_c60 數字是單機絕對峰值的正確參考。

### 32K 之後的長脈絡行為

網友也把 ladder 延伸到 64K/96K/128K，觀察到：

| size | decode（他們的資料） | TTFT（他們的資料） |
|---:|---:|---:|
| 65,536 | 112.8 | 21.5s |
| 98,304 | 87.7 | 48.6s |
| 131,072 | 69.9 | 106.3s |

所以那道牆是存在的，只是位置比 32K 遠很多。我們沒有自己重測這段；當成
網友的資料點看，不是這個 lab 的結果。

### 27B target+draft（待辦）

網友指出最近 dflash-mlx 的調校主要針對 27B-4bit target + `z-lab/Qwen3.6-27B-DFlash`
draft，所以 27B 路徑才是 v0.1.7 的主要優化目標，雖然 35B-A3B 路徑現在
也看起來健康。我們開始跑這個 sweep，但 `z-lab/Qwen3.6-27B-DFlash` 是個
gated HuggingFace repo，我們的 auth 沒設好；等這個解決後會在這裡加上
同方法論的 27B section。
